...
public class UpholsteryAddController {
    @FXML
    private Button add;
    ...
    void initialize() throws SQLException {
        ...
        add.setOnAction(actionEvent -> {
            ...
            try {
                tmp_index = fill.fillListInt("SELECT MAX(id_upholstery_material) FROM public.upholstery");
            ...
            if(Objects.equals(price.getText(), "")){
                sw.showError(headerError, "Заполните поле с ценой обивки");
                return;
            }

            if(Objects.equals(name.getText(), "")){
                sw.showError(headerError, "Заполните поле с названием обивки");
                return;

            }

            if(nameUn <= 0){
                sw.showError(headerError, "Выберете единицу измерения обивки");
                return;
            }

            if(Objects.equals(countUnit.getText(), "")){
                sw.showError(headerError, "Заполните поле с количеством обивки");
                return;
            }

            String sql = "INSERT INTO public.upholstery VALUES(" + index + ", " + "'" + price.getText() + "', " +
                    "'" + name.getText() + "', " + "" + nameUn + ", " + "'" + countUnit.getText() + "');";

            try {
                db.Insert(sql);

                sw.showInfo("Добавление обивки произошло успешно!", "");
